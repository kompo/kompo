...
class SelectPrepareValueForFrontTest extends EnvironmentBoot
{
    ...
    private function assert_attributes_are_correctly_transformed($form)
    {
        ...
        $this->assertEquals(3, $form->elements[1]->value[0]);
        $this->assertEquals(4, $form->elements[1]->value[1]);
        $this->assertCount(2, $form->elements[2]->value);
        $this->assertEquals(2, $form->elements[2]->value[0]);
        $this->assertEquals(5, $form->elements[2]->value[1]);
    }
    ...
    private function assert_single_parent_id_is_loaded_in_component_value($objSpecs, $formPositions)
    {
        ...
        foreach ($formPositions as $index) {
            $this->assertEquals(1, $form->elements[$index]->value);
        }
    }
